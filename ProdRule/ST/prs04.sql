SELECT       INTVL_START_DATE, CAST(BILL_RUN_TYP_IND AS VARCHAR(14) FOR SBCS DATA) AS BILL_RUN_TYP_IND, CAST(PRIM_IND AS VARCHAR(14) FOR SBCS DATA) AS PRIM_IND, CAST(BILL_PROD_RULE_ID AS VARCHAR(14) FOR SBCS DATA)  AS BILL_PROD_RULE_ID,      SPFNRL_ID, RLTD_CNUM_OID FROM        STHDSN6.TENGRA2.BILL_PRDRL_DATA WHERE       BACCT_PARTN_ID =:BACCT_PARTN_ID AND         BACCT_OID =:BACCT_OID ORDER BY    BILL_PROD_RULE_ID, INTVL_START_DATE DESC
...
curl -X GET http://131.116.44.144:1316/services/SYSIBMSERVICE/prs04/V1 -H "Content-Type: application/json" -u a472inb:INblatS2 | jq